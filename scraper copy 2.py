...
async def fetch_stream(length, sort_direction):
...
async def process_length(length, channels, semaphore):
    async with semaphore:
        print(f"\n[*] Starting extraction for Domain Length: {length}")
        tasks = [fetch_stream(length, sort_dir) for sort_dir in channels]
...
async def main():
    ...
    tasks = [process_length(length, channels, semaphore) for length in range(1, 64)]
